...
RSpec.describe Calendar::Week, type: :model do
  ...
  describe 'Get last of' do
    ...
    context '2 weeks of specific date' do
      ...
      it { expect(Calendar::Week.last_of(date)[0].week_number).to eq(52) }
      it { expect(Calendar::Week.last_of(date)[1].year.number).to eq(2016) }
      it { expect(Calendar::Week.last_of(date)[1].week_number).to eq(51) }
    end
  end
