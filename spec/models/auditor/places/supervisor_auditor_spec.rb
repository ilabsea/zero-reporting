...
RSpec.describe Auditor::Places::SupervisorAuditor, type: :model do
  ...
  let(:alert) { Alert::Place::SupervisorAlert.new od, report_setting, places }
  ...
  describe 'audit' do
    ...
    context 'process context alert' do
      before(:each) do
        ...
        allow(Alert::Place::SupervisorAlert).to receive(:new).with(od, report_setting, places).and_return(alert)
