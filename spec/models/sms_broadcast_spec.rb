  ...
  describe "#broadcast_to" do
    ...
    context "adding a sms to queue for every user who has phone number when it has active channels" do
      ...
      before(:each) do
        ...
        allow_any_instance_of(Channel).to receive(:is_connected?).and_return(true)
        allow(Sms::Message).to receive(:new).with('1000', 'Testing', instance_of(Channel), log_type).and_return(sms)
      end
