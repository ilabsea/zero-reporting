...
RSpec.describe Alerts::Place::SupervisorAlert, type: :model do
  let(:place) { create(:od) }
  ...
  context '#enabled?' do
    context 'false when it has no recipient type as OD or PHD or child_places is empty' do
      ...
      let(:alert) { Alerts::Place::SupervisorAlert.new nil, report_setting, [] }
    ...
    context 'true when it has recipient type as OD or PHD' do
      ...
      let(:alert) { Alerts::Place::SupervisorAlert.new nil, report_setting, HC.all }
  ...
  context '#variables' do
    context 'list all user phone numbers those are belongs to place' do
      let(:report_setting) { Setting::ReportSetting.new({ enables: ['sms'], x_week: 2, recipient_types: ['HC'] }) }
      let(:alert) { Alerts::Place::SupervisorAlert.new hc, report_setting, HC.all }
