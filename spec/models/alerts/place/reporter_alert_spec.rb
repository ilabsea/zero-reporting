...
RSpec.describe Alerts::Place::ReporterAlert, type: :model do
  context '#enabled?' do
    context 'false when it has no recipient type as HC' do
      let(:report_setting) { Setting::ReportSetting.new({ enables: [], x_week: 2, recipient_types: ['OD'] }) }
      let(:alert) { Alerts::Place::ReporterAlert.new nil, report_setting }
